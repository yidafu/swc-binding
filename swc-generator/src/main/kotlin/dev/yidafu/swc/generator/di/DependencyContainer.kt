...
class DependencyContainer(
    ...
    // 解析器
    private val _typeScriptParser by lazy { 
        TypeScriptParser(_swcNative, _swcGeneratorConfig) 
    }
    ...
    // 转换器
    private val _typeConverter by lazy { 
        TypeConverter(configuration) 
    }
    
    private val _interfaceConverter by lazy { 
        InterfaceConverter(configuration, _inheritanceAnalyzer) 
    }
    
    private val _typeAliasConverter by lazy { 
        TypeAliasConverter(configuration) 
    }
    
    private val _typeScriptToKotlinConverter by lazy { 
        TypeScriptToKotlinConverter(configuration, _inheritanceAnalyzer) 
    }
    
    // 处理器
    private val _kotlinADTProcessor by lazy { 
        KotlinADTProcessorFactory.createCombinedProcessor(_swcGeneratorConfig) 
    }
    
    // 性能优化器
    private val _performanceOptimizer by lazy { 
