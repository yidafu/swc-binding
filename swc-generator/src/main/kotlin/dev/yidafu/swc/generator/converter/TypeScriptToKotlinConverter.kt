...
class TypeScriptToKotlinConverter(
    ...
    private val interfaceConverter = InterfaceConverter(config, inheritanceAnalyzer)
    private val typeAliasConverter = TypeAliasConverter(config, inheritanceAnalyzer)
    ...
    fun convertDeclarations(
        ...
        return try {
            ...
            for (tsDeclaration in tsDeclarations) {
                val result = convertDeclaration(tsDeclaration)
    ...
    fun convertDeclaration(
        ...
        return when (tsDeclaration) {
            is TypeScriptDeclaration.InterfaceDeclaration -> {
                interfaceConverter.convert(tsDeclaration)
            }
            is TypeScriptDeclaration.TypeAliasDeclaration -> {
                typeAliasConverter.convert(tsDeclaration)
            }
        }
    }
     ...
     * 转换 TypeScript 类型
     */
    fun convertType(tsType: dev.yidafu.swc.generator.model.typescript.TypeScriptType): GeneratorResult<dev.yidafu.swc.generator.model.kotlin.KotlinType> {
